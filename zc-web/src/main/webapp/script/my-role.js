//给服务器发送请求获取分页数据(pageInfo),并在页面上显示分页效果(主题、导航条)
function showPage() {
...
function getPageInfo() {
    var ajaxRes = $.ajax({
        "url": "/role/search/keyword",
        "type": "post",
        "data": {
            "pageNum": (window.pageNum == undefined) ? 1 : window.pageNum,
            "pageSize": (window.pageSize == undefined) ? 6 : window.pageSize,
    ...
    var resultEntity = ajaxRes.responseJSON;
